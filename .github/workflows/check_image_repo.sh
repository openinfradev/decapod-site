...
do
  ...
  do
    ...
    if [  -z "$EXCEPTION_LIST" ]; then
      ...
      docker run --rm -i --net=host -v $(pwd)/decapod-base-yaml:/decapod-base-yaml --name generate ${DOCKER_IMAGE_REPO}/sktcloud/decapod-render:v3.1.0 helm2yaml/check_repo.py -m /$hr_file -r $VALIDATE_TARGET_REPO -t image 
    else
      # .github/workflows/check_repo.py -m $hr_file -r $VALIDATE_TARGET_REPO -t image -e $EXCEPTION_LIST
      docker run --rm -i --net=host -v $(pwd)/decapod-base-yaml:/decapod-base-yaml --name generate ${DOCKER_IMAGE_REPO}/sktcloud/decapod-render:v3.1.0 helm2yaml/check_repo.py -m /$hr_file -r $VALIDATE_TARGET_REPO -t image -e $EXCEPTION_LIST
    fi
